...
(async () => {
  ...
  try {
    ...
    await page.locator('#tmbox-language').selectOption('en');
    await page.locator('#tmbox-language-modal button[type=submit]').click();
    await page.waitForFunction(() => document.querySelector('#lcd-v2').textContent.includes('ASSIGN BOX'));
    assert.equal(posts.filter(path => path === '/v1/tmbox-v2/command').length, 0, 'Language grants no traffic authority');
    await page.reload();
    ...
    assert.equal(await page.evaluate(() => tmboxV2.nav.view.lookup_digits), '12', 'Admin push preserves unsent digits');
    assert.equal(posts.filter(path => path === '/v1/tmbox-v2/command').length, beforeDigits, 'Language changes are not traffic commands');
